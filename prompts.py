# Expert System Prompts for Routing
...
def check_manual_override(message: str) -> Tuple[Optional[str], str]:
    """
    Checks if a message starts with @intent (e.g., @code) and returns the intent if valid.
    """
    for intent in SYSTEM_PROMPTS.keys():
        prefix = f"@{intent}"
        if message.lower().startswith(prefix):
            return intent, message[len(prefix):].strip()
    return None, message
